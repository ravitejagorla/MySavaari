...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--country", type=str, default="India", help="Country name. Default: India")

    @transaction.atomic
    def handle(self, *args, **options):
        csv_file_path = options["csv_file"]
        country_name = options["country"]
        self.stdout.write(f"Importing geographic data from: {csv_file_path}")
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Found Country: {country.country_name}")
        )
        state_cache = {}
        city_cache = {}
        total_rows = 0
        ...
        try:
            with open(csv_file_path, mode="r", encoding="utf-8-sig", newline="") as file:
                ...
                required_columns = {"City", "Area", "Pincode", "District", "State",}
                if not reader.fieldnames:
                    raise CommandError("CSV file does not contain a header.")
                ...
                for row_number, row in enumerate(reader, start=2):
                    ...
                    try:
                        ...
                        if not pincode:
                            raise ValueError("Pincode is empty")
                        ...
                        city_key = (state.id, city_name.lower(),)
            ...
            raise CommandError("Unable to read the CSV file as UTF-8.")
        self.stdout.write("")
        ...
        self.stdout.write(f"Country: {country.country_name}")
        self.stdout.write(f"Rows Processed: {total_rows}")
        self.stdout.write(f"New States: {new_states}")
        self.stdout.write(f"New Cities: {new_cities}")
        self.stdout.write(f"New Areas: {new_areas}")
        self.stdout.write(self.style.SUCCESS("=" * 50))